...
def dictionar_complet_decriptat(dictionar_cuvinte: list,
                                individ: tuple) -> bool:
    return individ[1] is len(dictionar_cuvinte)


def obtine_indivizi_sortati(indivizi: list, text_codificat: str,
...
def decodifica(text_codificat: str, dictionar_cuvinte: list):
    indivizi = genereaza_indivizi(100)
    indivizi_sortati = obtine_indivizi_sortati(indivizi, text_codificat,
                                               dictionar_cuvinte)
    ...
    while not (
            dictionar_complet_decriptat(dictionar_cuvinte, indivizi_sortati[0]))\
        ...
        for i in range(len(elite)):
            ...
            copil1 = (copii[0],
                      get_fitness_score(text_codificat, dictionar_cuvinte,
                                        copii[0]))
            copil2 = (copii[1],
                      get_fitness_score(text_codificat, dictionar_cuvinte,
                                        copii[1]))
